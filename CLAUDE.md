...
企業のAI導入担当者が、目的に合った安全な社内コンテキストを準備できない課題を解決する。

社内情報はPDF、CSV、メモ、テンプレート、古い資料、個人知などに散らばっており、どの情報をAIに渡してよいのか判断しにくい。さらに、顧客情報や個人情報、古い資料、暗黙知が混ざるため、そのまま活用するのも危険になる。

このプロジェクトでは、Curatorが文書を分類し、MaskerがPIIや再識別リスクを検出して安全化または除外する。Strategistが目的に応じて「使える情報」「除外すべき情報と除外理由」「足りない情報」「人間に確認すべき質問」を整理し、Context Packageを生成する。

NotebookLM、RAG、資料作成など、さまざまな業務AIで活用できる安全な社内コンテキストを整える。候補文書は人間が生成前に確認し、MarkdownまたはNotebookLM向けbundleとして出力する。取り込んだ元ファイル（rawデータ）はCloud Storageのライフサイクル設定により14日後に自動削除する。
