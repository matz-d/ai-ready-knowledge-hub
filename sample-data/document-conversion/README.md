...
| `official-doc-pdf/synthetic-official-doc-table-assist-golden.pdf` | `official-doc-pdf` | Local synthetic fixture generated by `poc/document-conversion/official-doc-pdf/fixtures/generate-table-assist-golden.ts`. No external source and no customer data. | Compare-only fixture for Gemini table-assist evaluation. Intentionally not listed in stable P1-D fixtures because it is used to compare live table extraction behavior. | なし（合成・PII フリー） |
...
`official-doc-pdf/*.table-assist.expected.json` は、既存 public PDF に対する
compare-only table-assist golden set。PDF fixture は増やさず、
`poc/document-conversion/official-doc-pdf/compare/runCompare.ts` だけが読み込む。
stable P1-D の `STABLE_FIXTURES` には入れず、live Gemini 依存の gate にはしない。
...
**official-doc-pdf stable sidecars:** 公開 MHLW のうち `mhlw-overtime-limit-guide` / `mhlw-r07-model-work-rules` は、手書き stub を廃し **本線 pdf-parse**（`extractPdf` → `buildDocumentIr`）で再生成済み。scan-pdf と同じ raw baseline 方針。sidecar 再生成: `pnpm fixtures:official-doc-pdf:sidecars [<documentId>]`。`mhlw-labor-conditions-notice-general` は 2-column form table を pdf-parse だけでは復元できないため、stable CI が読む sidecar を **本線 table-assist merge 後 IR** に揃える。sidecar 再生成: `pnpm fixtures:official-doc-pdf:table-assist-sidecars [--fixture mhlw-labor-conditions-notice-general]`。`*.expected.json` は document-level truth として据え置き、golden を extractor 出力に合わせて緩めない。`synthetic-employment-context-with-pii` は手書きの PII value-retention fixture で、full-page text と最小 synthetic table locator を含むため自動再生成対象外。
