...
`D-P3-H-7` 着手ゲート 4「fixture #2〜#5 取得と inventory 追記」を **2026-05-21 に完了**。全 5 本が `scan-pdf/` に揃い、上記 Fixture inventory に記録済み。
...
| 5 | `degraded-scan-fail-closed.pdf` | fail-closed 発火確認 | ImageMagick で #2 を傾き+ノイズ劣化 → Ghostscript 120dpi 圧縮（6 MB） | ✅ |

次ステップ（M6-1 着手前ゲート残件）: `D-P3-H-7 Q3` PoC 実測（fixture 全件 × 3 回）と数値追補。
