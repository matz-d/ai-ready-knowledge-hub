...
| `scan-pdf/synthetic-unmaskable-pii-scan.pdf` | `scan-pdf` | Local deterministic synthetic fax-like scan generated for M6 W5b (2026-05-21). No external source and no customer data. 生成スクリプト: `poc/document-conversion/scan-pdf/fixtures/generate-synthetic-unmaskable-pii-scan.ts` | Repository fixture for live-smoke observation of `unmaskablePiiFindings.count > 0`. Fold bands obscure synthetic name / phone / address / My Number-like values while field labels remain visible; low-resolution rasterization, blur, and seeded scan noise keep exact span replacement unreliable. | **あり（合成 PII のみ）** |
| `scan-pdf/ocr-fail-closed-preflight.pdf` | `scan-pdf` | Derived from `scan-pdf/mhlw-labor-conditions-notice-blank-scan.pdf` via low-resolution + threshold + noise degradation (ImageMagick, 2026-05-21, <=5 MiB). 生成スクリプト: `poc/document-conversion/scan-pdf/fixtures/generate-ocr-fail-closed.sh` | Same public-source terms as the source MHLW scan fixture. **役割: OCR fail-closed 証跡専用（pre-flight route test / extractor fail-closed 観測用）**。413 size-limit 証跡は `degraded-scan-fail-closed.pdf` を使う。 | なし（白紙様式由来） |
| `scan-pdf/degraded-scan-fail-closed.pdf` | `scan-pdf` | Derived from `scan-pdf/mhlw-labor-conditions-notice-blank-scan.pdf` via ImageMagick (傾き・ノイズ) → Ghostscript 120dpi 圧縮 (2026-05-21, 6 MB). 生成スクリプト: `poc/document-conversion/scan-pdf/fixtures/generate-degraded.sh` | Same public-source terms as the source MHLW scan fixture. **役割: 5 MiB 超 → 413 size-limit 証跡（OCR fail-closed 用ではない）** — OCR fail-closed は ≤5 MiB の専用 fixture と pre-flight route test で確保。 | なし（白紙様式由来） |
...
`D-P3-H-7` 着手ゲート 4「fixture #2〜#5 取得と inventory 追記」を **2026-05-21 に完了**。その後、M6 W4b で OCR fail-closed 専用 fixture（≤5 MiB）、M6 W5b で deterministic unmaskable 合成 fixture を追加し、現在は全 7 本が `scan-pdf/` に揃っている。
...
| 5 | `degraded-scan-fail-closed.pdf` | **5 MiB 超 → 413 size-limit 証跡**（OCR fail-closed は ≤5 MiB の別 fixture + pre-flight route test） | ImageMagick で #2 を傾き+ノイズ劣化 → Ghostscript 120dpi 圧縮（6 MB） | ✅ |
| 6 | `ocr-fail-closed-preflight.pdf` | **≤5 MiB OCR fail-closed 証跡**（413 証跡には使わない） | #2 を低解像度化 + 二値化 + ノイズ付与（ImageMagick, `generate-ocr-fail-closed.sh`） | ✅ |
| 7 | `synthetic-unmaskable-pii-scan.pdf` | **live smoke `unmaskablePiiFindings.count > 0` 証跡** | 合成 fax page の field label を残して value を fold bands + low-resolution raster + blur + seeded noise で scan 化。本線 verifier 4 回確認後に採用（`generate-synthetic-unmaskable-pii-scan.ts`） | ✅ |
