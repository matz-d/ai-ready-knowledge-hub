...
**補足:** `POST /api/curator` の最小 Route Handler は W1-Close 後に追加済み。Week 2 の残作業は、Upload UI / Cloud Storage / Firestore 永続化と、この API 経路を実データ表示へ接続すること。
...
| 週 | 期間 | マイルストーン (仮置き) |
|---|---|---|
| Week 2 | 5/12-5/18 | Walking Skeleton (Cloud Storage + Firestore + Upload UI + Curator結果表示) |
