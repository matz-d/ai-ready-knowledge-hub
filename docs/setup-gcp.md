...
`PDF_TABLE_ASSIST_TASKS_QUEUE` と必要に応じて専用 worker token secret /
`PDF_TABLE_ASSIST_TASK_SIGNING_SECRET` を設定する。enqueue は
`{ docId, tenantId, actor, issuedAt }` を HMAC 署名し、worker は署名検証後にのみ
tenant context を信頼する。
