...
- **2026-05-29 (v3)**: `requires_masking` PDF の Masker 本線は **`D-P3-M-PDF-1` で後続完了**（§4.5 / Q5 の park は新規 upload では廃止、レガシー行のみ許容）。本 doc の M1 記述は H-2 時点の正本として残す。
- **2026-05-20 (v2)**: Phase 3-H-2 完了。M1〜M6 の実装・IAP 実機確認・CI gate 運用・Branch protection を反映。§13 Completion Snapshot を追加。
...
`D-P3-H-3` で「後続で確定する項目」とされた次の 3 点のうち、**heuristic 閾値**と **golden recall 判定**は Phase 3-H-2 で確定済み（`D-P3-H-5`、§7）。**feature flag の全 tenant 公開範囲**は dev tenant 限定のまま Phase 3-H-3 着手判断へ送る。

---

## 3.1 マイルストーン完了状況（2026-05-20）

| Milestone | 状態 | 主な成果物 / 証跡 |
|---|---|---|
| M1 薄い本線統合 | **完了** | `featureFlags.ts`、`pdfDocumentExtractor.ts`、`documentIrStorage.ts`、`uploadOrchestrator` PDF 分岐、`/api/documents` PDF 受理 |
| M2 観測データ蓄積 | **完了** | `conversionEvalStorage.ts`、`exportConversionEvalSamples.ts`、`document.convert` AuditEvent、dev Firestore 観測（`tmp/conversion-eval-samples-2026-05-20.jsonl`） |
| M3 Heuristic 閾値 | **完了** | `src/eval/conversion/heuristic/*`、`evalSafetyReadiness('heuristic')` + DLP bridge、閾値は `D-P3-H-5` |
| M4 Golden eval 雛形 | **完了** | 4 fixture の `*.expected.json`、`evalSemanticRetention`、`runConversionEvalGoldenCheck` |
| M5 CI gate | **完了** | `.github/workflows/conversion-eval.yml`、Branch protection ruleset `main required checks` |
| M6 H-3 引き継ぎ | **完了** | [docs/phase-3-h-3-direction.md](phase-3-h-3-direction.md)、`D-P3-H-6`（ドラフト） |

**検証コマンド（完了時点）:** `pnpm test` 479 pass、`pnpm typecheck` pass。

### IAP 実機確認（Cloud Run + IAP、tenant `m-grow-ai.com`）

| 観点 | 結果 |
|---|---|
| Runtime | GCP project `ai-ready-knowledge-hub`、revision `ai-ready-knowledge-hub-00018-xws` |
| Feature flag | `pdf-conversion-subtype-1` ON、`enabledTenants: ['m-grow-ai.com']`、`expiresAt: 2026-06-30` |
| 公的 PDF（`mhlw-labor-conditions-notice-general.pdf`） | `docId` `30b31987-fa2a-48d5-9c20-51402db67a82` — Curator `direct` → chunk 化、`status=curated`、`sourceSubtype=official-doc-pdf`、GCS `raw/{docId}/document-ir/v1.json`、`conversion_eval` health append |
| 合成 PII PDF（`synthetic-employment-context-with-pii.pdf`） | `docId` `74b16f16-2267-4fa5-bc15-13a56b791e92` — Curator `requires_masking` → **chunk 化しない**、`status=curated`、`maskingPending=true`、DocumentIR は GCS に保存（Masker 待ち） |
| M2-D 分布（別セッション） | `4420b2ca-b3e1-400f-974f-6f2c028c5d55`（労働条件通知書相当・`tableCandidates=78`）、`12ce8f8a-58d4-41b9-b760-8403b579e390`（`tableCandidates=116`）— heuristic DLP 実測の入力 |

### M5 CI gate 運用

| Stage | GitHub Actions | Branch protection |
|---|---|---|
| health | PR で必須。ジョブ名 `conversion-eval / health (required)` | ruleset **main required checks**（2026-05-20 有効化）で必須 |
| heuristic | PR で warning。`continue-on-error` + PR コメント（`<!-- conversion-eval-heuristic -->`） | 必須ではない |
| golden | `workflow_dispatch` + 月次 cron（`0 0 1 * *`）。PR / push では呼ばない | 必須ではない |
...
> **状態: 完了（2026-05-20）**。初回 CI golden の recall は低く、**`expected.json` チューニングが初回運用タスク**（§7.4、[docs/phase-3-h-2-monthly-review.md](phase-3-h-2-monthly-review.md)）。
...
`synthetic-employment-context-with-pii.pdf` の `expected.json` には **マスク後に残るべき非 PII 重要情報** を入れる。PII 自体の検出 recall は `safetyReadiness` / Masker 統合後の責務（本線では chunk 化しないため golden は sidecar IR 上の評価）。

### 7.4 初回 golden recall ベースライン（2026-05-20）

`pnpm tsx scripts/runConversionEvalForCi.ts --stage golden`（`DLP_DRY_RUN=true`）の初回サマリ。overall は案 B により **semantic retention の低 recall だけでは fail にならない**（運用レビュー用シグナル）。

| `documentId` | `keyFieldRecall` | `overall.status` | メモ |
|---|---:|---|---|
| `mhlw-overtime-limit-guide` | 0.19 | pass | expected フィールドが chunk 分割と未整合 |
| `mhlw-r07-model-work-rules` | 0.05 | pass | 同上 |
| `mhlw-labor-conditions-notice-general` | 0.06 | pass | IAP 実機 doc と同系 PDF |
| `synthetic-employment-context-with-pii` | 0.00 | warn | `context_package_readiness`（empty chunk）が warn に降格 |

**次の運用アクション:** [docs/phase-3-h-2-monthly-review.md](phase-3-h-2-monthly-review.md) Step 3〜4 で B/C/D 分類し、`expected.json` を chunk 連結テキストに合わせて更新する。

---

## 8. M5: CI gate 接続

> **状態: 完了（2026-05-20）**。必須 gate は GitHub **ruleset** `main required checks` で `conversion-eval / health (required)` を要求。
...
> **状態: 完了（2026-05-20）**（docs フェーズ）。実装は [docs/phase-3-h-3-direction.md](phase-3-h-3-direction.md) 着手ゲート後。
...
Phase 3-H-2 全体は次を満たしたら完了とする。**2026-05-20 時点ですべて達成済み。**

- [x] M1: `/api/documents` が feature flag ON の tenant 向けに PDF を受理し、`direct` 判定 PDF の chunk が Firestore に届く。`requires_masking` PDF は `curated` + `maskingPending: true` で停止する。
- [x] M1: DocumentIR snapshot が GCS `raw/{docId}/document-ir/v1.json` に保存される。
- [x] M1: `feature_flags/pdf-conversion-subtype-1` が dev tenant（`m-grow-ai.com`）先行で ON、期限 `2026-06-30`。
- [x] M2: `conversion_eval/{evalId}` に health stage 結果が append され、`documents/{docId}.latestConversionEvalId` から辿れる。
- [x] M2: AuditEvent `document.convert` action が記録される（`inferenceDestination` は未設定のまま H-3 予約）。
- [x] M3: subtype-1 の heuristic 閾値関数が実装され、`evalSafetyReadiness('heuristic')` が DLP を呼んで `maskableChunkRate` / `unmaskablePiiFindings` を算出する（`D-P3-H-5`）。
- [x] M4: 4 fixture 分の `*.expected.json` が配置され、`semanticRetention.keyFieldRecall` / `missingExpectedFields` が算出される。
- [x] M5: GitHub Actions に conversion eval ジョブが追加され、health gate が ruleset 必須化される。
- [x] M6: `docs/phase-3-h-3-direction.md` が起こされ、AuditEvent `inferenceDestination` 拡張仕様が固定される。
- [x] `pnpm test` / `pnpm typecheck` が通過する（完了時 479 tests pass）。
- [x] 新規依存導入が CLAUDE.md `minimumReleaseAge: 4320` に違反していない。

---

## 13. Completion Snapshot

- **Completed at:** 2026-05-20
- **Runtime project:** `ai-ready-knowledge-hub`
- **Cloud Run revision:** `ai-ready-knowledge-hub-00018-xws`
- **Feature flag:** `pdf-conversion-subtype-1` enabled for `m-grow-ai.com` until `2026-06-30`
- **Verified IAP upload:**
  - official doc (`mhlw-labor-conditions-notice-general.pdf`): `30b31987-fa2a-48d5-9c20-51402db67a82`
  - synthetic PII (`synthetic-employment-context-with-pii.pdf`): `74b16f16-2267-4fa5-bc15-13a56b791e92` — `requires_masking` + `maskingPending: true`、chunk なし
- **CI:**
  - `conversion-eval / health (required)` — PR 必須 + ruleset 必須
  - heuristic — PR warning コメント
  - golden — manual / monthly only
  - Branch protection — ruleset `main required checks`（`conversion-eval / health (required)`）
- **Carry-over to H-3:**
  - `inferenceDestination` on `document.convert`（subtype 2/3）
  - Masker integration for `requires_masking` PDF
  - golden `expected.json` tuning（初回 recall ベースライン §7.4）
  - `maskableChunkRate` redefinition after PII-bearing eval
  - feature flag 公開範囲拡大（subtype 1 / 2 / 3）
