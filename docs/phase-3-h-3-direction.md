...
- AuditEvent `document.convert`（`converterId` / `sourceSubtype` / `evalStatus`）。`inferenceDestination` は M2 時点で **予約・未設定**（[src/lib/audit/auditEvent.ts](../src/lib/audit/auditEvent.ts)）
...
### 4.2 Phase 3-H-3 で埋めるフィールド

| action | いつ `inferenceDestination` を付けるか | 付けない例 |
|---|---|---|
| `document.convert` | `converterId` が Vertex Gemini を呼んだとき（`slide-pdf` / `scan-pdf` の first-choice 成功時、fallback のみの場合は **未設定のまま**） | `official-doc-pdf` + `pdf-parse`、`pdf-parse-fallback` のみ |
| `document.export` | 既存どおり（変更なし） | — |
...
Phase 3-H-3 では、Vertex 呼出時に `inferenceDestination` を **同じ AuditEvent 行**に追加する。テスト方針: [src/lib/audit/__tests__/auditEvent.test.ts](../src/lib/audit/__tests__/auditEvent.test.ts) の「未設定」ケースを subtype 2/3 用に拡張する。
