...
| # | 問い | 答え | 一次成果物 |
|---|---|---|---|
| W1-1 | Genkit + Vertex AI で Curator 6 分類項目 + rationale の structured output が安定して取れるか | はい。10/10 件 Zod parse 通過。4 段フォールバック (structured / text JSON / format=json+schema / format=json) を実装し常用化 | `src/agents/curator/{schema,prompt,flow}.ts` |
...
- 固定デモ用の `src/demo/inventory.ts` / `strategistFixture.ts` は通常 UI から外し、独自 enum の混入余地を削除
- `scripts/generateInventorySnapshot.ts` を追加し、W1 回顧用 snapshot を `npm run inventory:snapshot` で実 LLM 出力に上書きできるようにした
...
- [ ] `npm run inventory:snapshot` を 1 回実行し、`docs/w1-artifacts/inventory.snapshot.json` を実 LLM 出力で初期化
- [x] `src/agents/curator/flow.ts` を Next.js Route Handler から呼ぶ最小経路を作る (`src/app/api/curator/route.ts`)
...
- `docs/w1-artifacts/inventory.snapshot.json` (W1 実 LLM 出力の回顧用 artifact)
...
固定デモ用 fixture を通常 UI に戻さず、`src/agents/curator/schema.ts` の enum を正本にして
Route Handler / Firestore / UI の表示型をつなぐ。
