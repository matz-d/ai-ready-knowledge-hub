...
5. 14日を超えた chunkless な `curated`/`direct` 文書（特に `.xlsx`）は、生成時の本文 fallback（`raw/` 読み）が失われ `CONTEXT_PACKAGE_GCS_BODY_UNAVAILABLE` になり得る。これは PII-at-rest 最小化を優先した上での **許容する劣化**とし、必要時は再アップロードで復旧する。本線 PDF（chunk 永続化済み）と `ai_safe`（`masked/` 参照）は非該当。
