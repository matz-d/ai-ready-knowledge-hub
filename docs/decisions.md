...
**決定**: `poc/w1/` は削除し、Curator/Masker の Zod schema・prompt・Genkit flow を `src/agents/` 配下の正本へ昇格させる。UI 層 (`src/demo/inventory.ts` ほか) は R5 確定 enum (`DocumentTypeEnum` / `BusinessDomainEnum` / `SensitivityEnum` / `FreshnessEnum` / `AiUsePolicyEnum`) を直接参照し、独自の英語 enum 体系を持たない。

**背景:**
W1 の見た目作り込みフェーズで `src/demo/inventory.ts` に独自 enum が混入し、PoC で R5 確定した日本語 enum と二重管理になりかけていた。`docs/open-questions.md R5` の確定値を破壊しないよう、Walking Skeleton 着手前に正本化する。
...
src/demo/
  inventory.ts                    # InventorySnapshotEntry = CuratorOutputResult + fileName + maskerEvaluation
  strategistFixture.ts            # Strategist 出力フィクスチャ (Sensitivity は curator schema を参照)
scripts/
  runCurator.ts / runCuratorAll.ts / runMaskerRisk.ts
  generateInventorySnapshot.ts    # sample-data/accounting-office を curatorFlow に通し src/demo/inventory.snapshot.json を生成
sample-data/
...
- `npm run inventory:snapshot` — 実 LLM 出力を JSON snapshot として src/demo に保存
- `npm run typecheck` / `npm run build` — Next.js 込みで通過確認済み

**狙い:**
- 「見た目だけ」「擬似的に固定値で動かしている」という疑念を、`inventory:snapshot` の実行ログで打ち返せる構造にする
- Walking Skeleton 着手時に、Server Action / Route Handler から `src/agents/curator/flow.ts` を呼べばそのまま実体配線になる
