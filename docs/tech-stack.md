...
| 実行環境 | **Cloud Run** | ハッカソン必須要件、Buildpacks + Next.js standalone 出力で配信 |
...
Cloud Run へは Dockerfile を書かず、`next.config.ts` の `output: 'standalone'` と Cloud Run Buildpacks に任せる。Dockerfile を持たないことでハッカソン期間中の Node バージョン追従コストを下げる。
...
- `next.config.ts` で `output: 'standalone'` を指定し、Cloud Run Buildpacks に `npm start` させる
