...
- Firestore Native mode の document hard limit は **1 MiB (1,048,576 bytes)**。field value は **1 MiB - 89 bytes** が上限。
- Phase 2 は `documents/{docId}/chunks/{chunkId}` に `text` と `maskedText` を inline 保存するため、1 MiB に対する保守的な余白として **1 chunk document 推定 500 KiB 以下**を invariant にする。
- 500 KiB を超える chunk は `replaceChunksForDocument` の事前 invariant 検査で reject し、既存 chunk subcollection には書き込まない。Phase 2 では oversized sheet の自動分割はしない。

**撤退条件:** chunk 数が document あたり 200 を超える、または 500 KiB を超える sheet が典型ユースケースとして出てきた場合、Phase 3 以降で sheet 内 row-range split か C 案（chunk body / masked body の GCS 外出し）に移行する。
...
- `replaceChunksForDocument` は **全 chunk を事前検証してから** batch write する。1 chunk でも invariant 違反（parent/status/hash/maskedText/size 等）があれば CLI は non-zero で終了し、新 chunk は一切書かない（all-or-nothing）。
- demo runbook に明示
...
**xlsx partial failure policy:**
- Phase 2 の `.xlsx` 解析は workbook 全体を `xlsx` library で読み込んでから sheet 単位 chunk を生成する。workbook 自体が壊れている、OOXML zip ではない、または parser が例外を投げる場合は **upload / `chunks:regenerate` とも失敗**にする。
- sheet 単位で recoverable な問題が取れる場合のみ、当該 chunk の `extractionWarnings` に warning を残す。Phase 2 では壊れた sheet だけを silently skip して partial success にはしない。
- masking / Firestore 書き込み段階で一部 chunk が失敗した場合も partial success は認めない。CLI は non-zero、Firestore 書き込みは事前検証 all-or-nothing。削除フェーズだけは既存方針どおり、write 成功後に失敗すると stale chunk が一時的に残りうる。
...
7. 推定 Firestore payload size は **500 KiB 以下**。Firestore の 1 MiB document limit に対する Phase 2 の安全マージンとして扱う。
