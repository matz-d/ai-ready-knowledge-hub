...
| P1-D | Extraction & Masking Quality Gate | 「安全に止める」だけでなく「止めすぎない」「構造化データ精度が十分」を示す | 初期 stable quality gate と local mixed PDF check は実装済み。public blank-form / guide sidecar へ structured expectations と `expectedFieldTiers` を追加し、`coreFieldRecallAverage` と deterministic zero-check candidates を分離済み。scan-pdf sidecar は raw OCR baseline に戻し、table row locator 不在も実シグナルとして扱う | 残り synthetic / slide sidecar に core/extended tier と source-document intent ベースの structured expectations を広げる。curator over-restriction は live-only、`emptyChunkCount` の既知 1 件を解消してからゼロ指標 blocker 化を検討 |
