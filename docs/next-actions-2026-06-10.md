...
| P1-D | Extraction & Masking Quality Gate | 「安全に止める」だけでなく「止めすぎない」「構造化データ精度が十分」を示す | 初期 stable quality gate と local mixed PDF check は実装済み。public blank-form sidecar へ structured `expectedValues` / `expectedTableCells` を追加し、`mhlw-labor-conditions-notice-blank-scan` も stable eval 対象化済み。scan-pdf sidecar は raw OCR baseline に戻し、table row locator 不在も実シグナルとして扱う | core/extended tier を expected schema に足し、source-document intent ベースで structured expectations を広げる。curator over-restriction は live-only、決定論的ゼロ指標は早期 blocker 候補 |
