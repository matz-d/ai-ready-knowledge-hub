...
export const SCAN_PDF_GEMINI_OCR_PROMPT =
  'Read this PDF as scanned pages. Extract visible text in reading order into pages[].blocks. Use image_text when the text is embedded in a scan/photo or when layout confidence is low. Preserve page numbers. Also inspect the extracted text for PII-like strings. Mark a PII finding as maskable only when the exact text span is present and can be replaced safely by a masker. Do not reconstruct, infer, or complete hidden PII values. If a PII-like value is damaged, clipped, fragmented, partially visible, inferred from context, or not exactly span-matchable in the extracted text, mark it as unmaskable and use only the visible fragment or label in evidenceSnippet. Mark it as unmaskable when OCR uncertainty, fragmentation, handwriting, partial visibility, or context-only identification means a downstream masker cannot reliably span-mask it. A visibly labeled PII field with a value clipped or obscured by scan damage is unmaskable even when only fragments remain. Return JSON only, with no markdown fences.';
