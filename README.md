...
**フェーズ**: Phase 3-E（Processing Boundary + Cloud DLP Trust Modes）完了。次は Phase 3-H（Document Conversion PoC）を前倒しで進める。`cloud-managed` を標準 ProcessingProfile として固定し、Cloud DLP provider 固定値、Context Package export の `purposeBinding`、`cloud-sanitized-ingress` contract-only 方針、Document Conversion Eval 契約を docs / 型 / tests で同期済み。Phase 3-D の CI/CD + IAP + AuditEvent も完了済みで、commit push → 自動 CI → Artifact Registry → Cloud Run の継続デプロイと Cloud IAP による社内限定公開が稼働中。
...
- **Phase 3-H**: Document Conversion PoC。PDF / Slide / 画像 / Office 系ファイルを `DocumentIR` / `KnowledgeChunk` 相当へ変換し、Phase 3-E で固定した Conversion Eval の health check を実装する。正本は `docs/phase-3-h-direction.md`。
- **提供形態の整理**: Managed SaaS / Dedicated SaaS / Customer-managed / Sanitized ingress の位置づけを `docs/offering-model.md` に固定済み。商用本命は Dedicated SaaS / Private deployment とし、Managed SaaS は入口、`cloud-sanitized-ingress` は将来 Enterprise option として扱う。
- **Phase 3-F**: Document Conversion を含むデモ polish・動画シナリオ・見栄え調整。Phase 3-H の後に回す。
