...
- **Masker 本線統合（PDF 経路）**: 実装完了（`D-P3-M-PDF-1`、branch `feat/masker-pdf-mainline`）。`requires_masking` PDF を text 経路と同型に `ai_safe` / `restricted` 終端まで処理し chunk 化・Context Package 接続。残: live smoke 証跡・`unmaskablePiiFindings` 閾値再評価・scan-pdf 公開範囲拡大（`D-P3-H-7 Q4`、別 decision）。
- **Ingest 拡張（起票済み・未着手）**:
